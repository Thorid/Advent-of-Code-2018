class Unit():
    def __init__(self, coordinates, side, iden):
        self.hp = 200
        self.attackPower = 3
...
def parseUnits(file):
    ...
    for y in range(len(fileArray)):
        for x in range(len(fileArray[y])):
            if fileArray[y][x] == 'G':
                unitsArray.append(Unit([x,y],'Goblin', goblinCounter))
                goblinCounter += 1
            elif fileArray[y][x] == 'E':
                unitsArray.append(Unit([x,y],'Elf', elfCounter))
                elfCounter += 1
    return unitsArray
...
unitsArray = parseUnits(inputFile)
currentMap = updateMap(mapDict,unitsArray)
battleEnded = False
roundNo = 0
while True:
